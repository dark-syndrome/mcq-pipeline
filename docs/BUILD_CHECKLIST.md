...
- **Critic Criteria Heatmap uses a raw HTML `<table>`, not Recharts** (Session 9, §4.2). The heatmap is a 2-D grid where each cell needs an individually computed HSL background color (green↔red for pass rate); Recharts has no built-in heatmap primitive. `CriticHeatmap.tsx` renders a plain `<table>` with inline styles and skips `ChartCard` (which wraps children in `ResponsiveContainer`). All other Session 9 charts use Recharts + ChartCard as normal.
- **Run History "Inspect" drawer fetches MCQs via existing `queryMcqs` IPC** (Session 9, §4.2). No new IPC needed — the drawer queries `{ runIds: [run_id], passedOnly: true, limit: 50 }` against the already-wired `db:queryMcqs` handler. Run metadata (gen#, counts, cost) comes from the `runHistory` row itself.
- **Run History "Export" action uses the existing `export:run` IPC** (Session 9, §4.2). Fetches accepted MCQs for the run via `queryMcqs`, then calls `window.api.export.run()` with `{ format: 'json' }` which opens the OS Save dialog. Reuses the same export path as the Files tab — no new IPC.
...
- **Eval Set storage uses JSON files in `eval-sets/`, not SQLite** (Session 10, spec §7). The Eval Set is purely GUI-owned annotation data (ratings, confirmed flags, notes) — keeping it in JSON outside the Python-owned `runs.db` keeps the two-store separation clean and makes the eval files portable (importable/exportable as self-contained JSON). `gui/electron/evalset.ts` owns all CRUD; pipeline never touches `eval-sets/`.
- **Critic criteria badge simplified to "✓ Critic accepted" for accepted questions** (Session 10, §7.3). The DB stores `critique_json` only for `passed=0` rows — accepted questions have `NULL` critique. `BrowseView.tsx` shows a single "✓ Critic accepted" badge instead of 13 individual criterion badges when `critique_json` is absent. Upgrade possible if/when the pipeline persists the full `CritiqueResult` for passing questions.
- **Quality Summary in Eval Set tab is conditionally rendered** (Session 10, §7.3). The Summary card only appears once at least one question has been annotated (`confirmed !== null || rating > 0`) — avoids showing empty stat cards and a misleading "0% false positives" on a brand-new set.
...
| 9 | Dashboard sub-tabs 2–4 (quality heatmap, cost/tokens, run history) | 4.2 | 6 | `[x]` |
| 10 | Eval Set tab (browse/annotate, table, management) | 7 | 7 | `[x]` |
...
| `question_rejected` event | §8.2 | `[x]` | Emitted post-reframe for the FINAL rejected set (count matches `run_complete.rejected`); `failure_class` reuses `reframer.classify_rejection`. GUI: `RejectedFeed.tsx` + reducer `rejected[]` |
| Per-stage cost for `generate`/`critic` on `stage_done` | §8.2 | `[x]` | `generate` + `reframe` `stage_done` now carry aggregate `tokens_in/out/cost`. `critic` has no timeline stage — stays in `run_complete.cost_breakdown.critic`. GUI session-cost reconcile updated to add critic-only at completion |
| Per-Bloom-level temperature config schema | Open #5 (P2) | `[x]` | `BloomTemperatures` model + `bloom_temperatures` in config.yaml (spec §3.4 defaults). **Now consumed by the Generator** — see below |
| Per-Bloom-level temperature consumed by Generator | Open #5 (P2) | `[x]` | Faithful per-Bloom multi-call: one Generator call per *active* Bloom level (union of `concept.supported_bloom_levels`), each at its `bloom_temperatures.<level>`. Deficit split even+remainder-to-highest-support across levels each retry attempt. Degenerate (no Bloom levels) → single flat-temp call. `generate_mcqs` gained `temperature_override`/`bloom_target`; `{bloom_directive}` placeholder added to `prompts/generator.txt`. Helpers unit-tested in `tests/test_bloom_generation.py` |
